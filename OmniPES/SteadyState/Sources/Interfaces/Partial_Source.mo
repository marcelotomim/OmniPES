...
partial model Partial_Source
  ...
  parameter Units.PerUnit Vmax = 2.0 if voltage_limits "Maximum voltage limit" annotation(Evaluate=true, HideResult=false, Dialog(group="Limits", enable=voltage_limits));
  parameter Units.PerUnit Vmin = 0.5 if voltage_limits "Minimum voltage limit" annotation(Evaluate=true, HideResult=false, Dialog(group="Limits", enable=voltage_limits));
