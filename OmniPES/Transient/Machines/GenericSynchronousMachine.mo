...
model GenericSynchronousMachine
  ...
  OmniPES.Circuit.Interfaces.PositivePin terminal annotation(
    Placement(transformation(origin = {-95, 32}, extent = {{-12, -12}, {12, 12}}), iconTransformation(origin = {-100, 0}, extent = {{-10, -10}, {10, 10}})));
  ...
  replaceable OmniPES.Transient.Machines.Interfaces.Restriction restriction annotation(
    Placement(transformation(origin = {62, 69}, extent = {{-19, -19}, {19, 19}}))) constrainedby Interfaces.Restriction(param = specs) annotation(
  ...
  replaceable OmniPES.Transient.Machines.Interfaces.PartialElectrical electrical(smData = smData) annotation(
    Placement(transformation(origin = {-15.5, 2.5}, extent = {{-20.5, -20.5}, {20.5, 20.5}})));
equation
  ...
  connect(electrical.Pe, inertia.Pe) annotation(
    Line(points = {{7, 15}, {41, 15}}, color = {0, 0, 127}));
  connect(avr.Efd, electrical.Efd) annotation(
    Line(points = {{-61, -10}, {-38, -10}}, color = {0, 0, 127}));
  connect(electrical.Vt, avr.Vctrl) annotation(
    Line(points = {{7, -10}, {17, -10}, {17, -44}, {-90, -44}, {-90, -16}, {-83, -16}}, color = {0, 0, 127}));
  connect(inertia.delta, electrical.delta) annotation(
    Line(points = {{85, 15}, {90, 15}, {90, 33}, {-56, 33}, {-56, 15}, {-38, 15}}, color = {0, 0, 127}));
  connect(terminal, electrical.terminal) annotation(
    Line(points = {{-95, 32}, {-69, 32}, {-69, 2.5}, {-38, 2.5}}, color = {0, 0, 255}));
  annotation(
