...
equation
  ...
  if sreg_on then
    connect(sreg.signalBus.omega, inertia.omega) annotation(
    ...
    Line(points = {{-11, -20}, {75, -20}, {75, -44}}, color = {255, 204, 51}, thickness = 0.5));
  else
    der(inertia.Pm) = 0;
  end if;
  ...
  if avr_on then
    ...
    if pss_on then
      connect(inertia.omega, pss.signalBus.omega) annotation(
    Line(points = {{89, -9}, {94, -9}, {94, -69}, {15, -69}}, color = {0, 0, 127}));
      connect(electrical.signalBus, pss.signalBus) annotation(
    Line(points = {{-11, -20}, {-11, -41}, {15, -41}, {15, -69}}, color = {255, 204, 51}, thickness = 0.5));
