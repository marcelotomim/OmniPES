...
model Test_Radial_System_CTRL
  ...
  parameter OmniPES.Transient.Machines.RestrictionData gen1_specs(Psp = 0, Qsp = 0.0, Vsp = 0.9, theta_sp = 0) annotation(
  ...
  OmniPES.Circuit.Basic.SeriesImpedance impedance2(x = 0.01) annotation(
    Placement(transformation(origin = {-66, 18}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  OmniPES.Transient.Machines.GenericSynchronousMachine SM(smData = gen1_data, specs = gen1_specs, redeclare OmniPES.Transient.Machines.Interfaces.Model_1_0_Electric electrical, redeclare OmniPES.Transient.Machines.Interfaces.Restriction_PV restriction, redeclare IEEE_AC4A avr, redeclare OmniPES.Transient.Controllers.PSS.NoPSS pss, redeclare OmniPES.Transient.Controllers.SpeedRegulators.ConstantPm sreg) annotation(
  ...
  OmniPES.Circuit.Switches.Fault fault(X = 1e-4, t_off = 100.2, t_on = 100.1) annotation(
  ...
  OmniPES.Circuit.Interfaces.Bus bus1 annotation(
    Placement(transformation(origin = {-90, 20}, extent = {{-8, -8}, {8, 8}})));
  ...
  model IEEE_AC4A
    ...
    Modelica.Blocks.Sources.Constant const(k = 0) annotation(
      Placement(transformation(origin = {-188, 2}, extent = {{-10, -10}, {10, 10}})));
    ...
    Modelica.Blocks.Sources.Ramp ramp(height = 0.3, duration = 7, startTime = 1) annotation(
      Placement(transformation(origin = {-174, 38}, extent = {{-10, -10}, {10, 10}})));
    Modelica.Blocks.Math.Sum sum1(nin = 2) annotation(
      Placement(transformation(origin = {-94, 2}, extent = {{-10, -10}, {10, 10}})));
  equation
    connect(add3.u3, Vsad) annotation(
      Line(points = {{-56, -8}, {-68, -8}, {-68, -60}, {-120, -60}}, color = {0, 0, 127}));
    connect(const.y, Vref.u) annotation(
      Line(points = {{-177, 2}, {-167, 2}, {-167, 2}, {-167, 2}}, color = {0, 0, 127}));
    ...
    connect(Vref.y, sum1.u[1]) annotation(
      Line(points = {{-143, 2}, {-106, 2}}, color = {0, 0, 127}));
    connect(ramp.y, sum1.u[2]) annotation(
      Line(points = {{-162, 38}, {-106, 38}, {-106, 2}}, color = {0, 0, 127}));
    connect(sum1.y, add3.u2) annotation(
      Line(points = {{-82, 2}, {-70, 2}, {-70, 0}, {-56, 0}}, color = {0, 0, 127}));
  end IEEE_AC4A;
equation
  ...
  connect(impedance2.p, bus2.p) annotation(
    Line(points = {{-56, 18}, {-46.8, 18}}, color = {0, 0, 255}, thickness = 0.5));
  connect(bus3.p, SM.terminal) annotation(
    Line(points = {{49.88, 18.8}, {62.88, 18.8}, {62.88, 18}, {68, 18}}, color = {0, 0, 255}, thickness = 0.5));
  ...
  connect(voltageSource.p, bus1.p) annotation(
    Line(points = {{-110, 6}, {-110, 18}, {-90, 18}}, color = {0, 0, 255}, thickness = 0.5));
  connect(bus1.p, impedance2.n) annotation(
    Line(points = {{-90, 18}, {-76, 18}}, color = {0, 0, 255}, thickness = 0.5));
protected
  annotation(
