...
model Test_Single_Machine
  ...
  Circuit.Sources.VoltageSource voltageSource(angle = 0, magnitude = 1.05) annotation(
    Placement(transformation(origin = {-32, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  parameter Transient.SynchronousMachines.RestrictionData gen1_specs(Psp = 9e7, Qsp = 5e7, theta_sp = 0) annotation(
  ...
  //
  Transient.SynchronousMachines.GenericSynchronousMachine SM(smData = gen1_data, specs = gen1_specs, redeclare OmniPES.Transient.SynchronousMachines.Interfaces.Model_2_2_Electric electrical, redeclare OmniPES.Transient.SynchronousMachines.Interfaces.Restriction_PQ restriction, avr_on = true, redeclare FieldStep avr) annotation(
    Placement(transformation(origin = {44, 0}, extent = {{-10, -10}, {10, 10}})));
  parameter Transient.SynchronousMachines.SynchronousMachineData gen1_data(D = 0, H = 6.5, Nmaq = 1, Ra = 0, T1d0 = 8, T1q0 = 0.4, T2d0 = 0.03, T2q0 = 0.05, X1d = 0.3, X1q = 0.55, X2d = 0.25, X2q = 0.25, Xd = 1.8, Xl = 0.2, Xq = 1.7, MVAb = 1e8) annotation(
...
equation
  ...
  connect(voltageSource.p, bus.p) annotation(
    Line(points = {{-22, 0}, {0, 0}}, color = {0, 0, 255}));
protected
  annotation(
